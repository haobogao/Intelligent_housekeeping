...
int main( void )
{
  // Turn off interrupts  关闭终端，为了保证系统环境正常
  osal_int_disable( INTS_ALL );

  // Initialization for board related stuff such as LEDs 初始化外部硬件
  HAL_BOARD_INIT();

  // Make sure supply voltage is high enough to run 测试电压能否启动
  zmain_vdd_check();

  // Initialize board I/O 初始化IO
  InitBoard( OB_COLD );

  // Initialze HAL drivers 硬件驱动初始化
  HalDriverInit();
  ...
  // Initialize the operating system 操作系统的初始化
  osal_init_system();
